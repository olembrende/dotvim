...
function! MakeMatlabComment()
   ...
   if (l:parenPos >= 0)
      call append(l:nextLine-1, "\t\% INPUTS")
      let l:nextLine=l:nextLine+1
   ...
   " Output parameters
   if (l:eqPos >= 0)
      call append(l:nextLine-1, "\t\% OUTPUTS")
      let l:nextLine=l:nextLine+1
   ...
   " Author lineStr
   call append(l:nextLine-1, "\t\% Made by: Ole Martin Brende, InPhase Solutions AS")
   let l:nextLine=l:nextLine+1
   call append(l:nextLine-1, "\t\% Date: " . system('date /t'))
   let l:nextLine=l:nextLine+1
